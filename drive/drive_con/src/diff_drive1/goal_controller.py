...
class GoalController:
    ...
    def get_goal_distance(self,pose, goal):
        if goal is None:
            return 0
        diffX = pose.x - goal.x
        diffY = pose.y - goal.y
    ...
    def at_goal(self,pose, goal):
        if goal is None:
            return True
        d = self.get_goal_distance(pose, goal)

        return d < self.linear_tolerance 

    def get_velocity(self,pose, goal, dT):
        desired = Pose()

        goal_heading = atan2(goal.y - pose.y, goal.x - pose.x)
        a = -pose.theta + goal_heading
        ...
        theta_1 = self.normalize_pi(pose.theta - goal.theta)
        b = -theta_1 - a
        ...
        if abs(d) < self.linear_tolerance:
            desired.xVel = 0
            desired.thetaVel = self.kB * pose.theta
        else:
